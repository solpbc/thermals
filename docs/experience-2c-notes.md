...
# thermals experience layer — delivery notes
...
The design SOT, the renders, and the design-gate record live in sol pbc's
...
## Two coordination items for the worker owner
...
1. **Byte-clean headers on asset responses — launch-blocking for the AC.**
   ...
   carries `Cache-Control: …, no-transform` — this is exactly what sol pbc's
   holding worker guards against. Without it the **byte-clean /
   ...
   `Permissions-Policy` incl. `interest-cohort=()`). Reference: the holding
   worker's `SECURITY_HEADERS` + `withSecurityHeaders` helpers. My assets themselves make **zero** third-party
...
2. **Avatars are ON** (`USE_AVATARS = true` in `app.js`) — the fast-follow to the
   v1 monogram launch shipped. A rook carrying a
